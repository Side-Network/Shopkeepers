...
public class BlockVillagerSpawnListener implements Listener {
	...
	private boolean isSpawningBlocked(EntityType entityType, World world) {
		if (entityType == EntityType.VILLAGER) {
			return Settings.blockVillagerSpawns
					&& (Settings.blockVillagerSpawnsWorlds.isEmpty()
							|| Settings.blockVillagerSpawnsWorlds.contains(world.getName()));
			...
			return Settings.blockWanderingTraderSpawns
					&& (Settings.blockWanderingTraderSpawnsWorlds.isEmpty()
							|| Settings.blockWanderingTraderSpawnsWorlds.contains(world.getName()));
		} else {
			return false;
		}
	}
	...
	void onSpawn(CreatureSpawnEvent event) {
		...
		var world = Unsafe.assertNonNull(event.getLocation().getWorld());
