...
public final class CompatProviderImpl implements CompatProvider {
	...
	public void setZombieNautilusVariant(LivingEntity zombieNautilus, NamespacedKey variant) {
		ZombieNautilus.Variant variantValue = Registry.ZOMBIE_NAUTILUS_VARIANT.get(variant);
		if (variantValue == null) {
	...
	public NamespacedKey cycleZombieNautilusVariant(NamespacedKey variant, boolean backwards) {
		...
		Registry<ZombieNautilus.Variant> registry = Unsafe.castNonNull(Registry.ZOMBIE_NAUTILUS_VARIANT);
		return RegistryUtils.cycleKeyed(registry, variantValue, backwards).getKeyOrThrow();
	}
}
